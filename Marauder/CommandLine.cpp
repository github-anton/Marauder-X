...
void CommandLine::runCommand(String input) {
  ...
  // Clear APs
    ...
    if (ss_sw == -1) {
      ...
      if (en_sw != -1)
        result = settings->saveSetting<bool>(setting_name, true);
      else if (da_sw != -1)
        result = settings->saveSetting<bool>(setting_name, false);
      else
        return;
  ...
  if (cmd_args.get(0) == LIST_AP_CMD) {
    ...
    if ((ap_sw != -1) && (pw_sw != -1)) {
      ...
      String ssid = settings->loadSetting<String>("ClientSSID");
      String pw = settings->loadSetting<String>("ClientPW");
